        ...
        if(s.group[e.ke].activeMonitors[e.id].childNode){
            ...
            if(k.fileExists===true){
                ...
                if(config.childNodes.enabled === true && config.childNodes.mode === 'child' && config.childNodes.host){
                    console.log("send event for completed recording");
                    const response = {
                    ...
                    console.log("send event for completed recording else part");
                    ...
                    if(storageIndex){
                        ...
                        console.log("setDiskUsedForGroup");
                    }
                    console.log("insert before Completed",s.onBeforeInsertCompletedVideoExtensions);
                    s.onBeforeInsertCompletedVideoExtensions.forEach(function(extender){
                        extender(e,k)
                    })
                    s.insertDatabaseRow(e,k,callback)
                    // s.trimVideo(e,k,callback)
                    console.log("insert Completed",s.insertCompletedVideoExtensions);
        ...
        if(forceCheck === true || config.insertOrphans === true){
            ...
            fs.readdir(videosDirectory,function(err,files){
                if(files && files.length > 0){
                    ...
                    var count = 0;
                    var startWithTrime = files.filter((file_)=>{
                        if (file_.startsWith("trim-")) count++
                    })
                    console.log(files.length - checkMax - count ,files.length-files);
                    var fiveRecentFiles = files.slice(files.length - checkMax - count ,files.length - count )
                    console.log("fiveRecentFiles",fiveRecentFiles);
                    var completedFile = 0
                        ...
                        if(/T[0-9][0-9]-[0-9][0-9]-[0-9][0-9]./.test(filename)){
                            s.knexQuery({
                                ...
                                if(!err && (!rows || !rows[0])){
                                    ++orphanedFilesCount
                                    var video = rows[0]
                                    console.log("orphanedVideoCheck");
